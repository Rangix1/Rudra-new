...
module.exports.config = {
    ...
    version: "1.0.2",
    hasPermssion: 0,
    credits: "Fixed By Rudra + Typing by ChatGPT",
    description: "goibot",
...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
    ...
    if (event.body?.toLowerCase().startsWith("bot")) {
        ...
        // Hide typing indicator in the group
        api.sendTypingIndicator(threadID, false);

        return api.sendMessage(msg, threadID, messageID);
    }

    // Reply for inbox messages
    if (event.threadID === senderID) {
        const rand = tl[Math.floor(Math.random() * tl.length)];

        // Show typing indicator in inbox
        api.sendTypingIndicator(threadID, true);
        ...
        // Hide typing indicator in inbox
        api.sendTypingIndicator(threadID, false);
